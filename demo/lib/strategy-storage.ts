...
export function getDraftFromStorage(): StrategyTemplate | undefined {
  if (typeof window === "undefined") return undefined

  try {
    const stored = localStorage.getItem('strategy-draft')
    if (!stored) return undefined
    return JSON.parse(stored)
  } catch (error) {
    console.warn('Failed to parse strategy draft:', error)
    return undefined
  }
}
